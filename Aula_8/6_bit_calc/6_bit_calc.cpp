// Porte do 6_bit_calc.ino (ESP32) para rodar como programa nativo em um
...
//
// Compilar:  g++ -std=c++17 -O2 -o 6_bit_calc 6_bit_calc.cpp
// Executar:  ./6_bit_calc
...
void print_operation_params(const string& op, uint16_t A, uint16_t B) {
  ...
  printf("Operacao: %s\n", op.c_str());
  cout << "A: " << bitset<6>(A & 0x3F) << endl;
  cout << "B: " << bitset<6>(B & 0x3F) << endl;
}

uint16_t handle_addition(uint16_t A, uint16_t B, bool* overflow) {
  uint16_t result = A + B;
  *overflow =
      (((A & 0x20) == (B & 0x20)) && ((result & 0x20) != (A & 0x20)));
  return result;
}
    ...
    while (true) {
      ...
      if (c == '\n') {
        if (currentLine.find("GET /calc?") != string::npos) {
          ...
          cout << "Resultado: " << bitset<6>(result & 0x3F) << endl;
          if (overflow) {

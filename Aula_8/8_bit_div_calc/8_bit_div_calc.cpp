// Porte do 8_bit_div_calc.ino (ESP32) para rodar como programa nativo em um
...
//
// Compilar:  g++ -std=c++17 -O2 -o 8_bit_div_calc 8_bit_div_calc.cpp
// Executar:  ./8_bit_div_calc
...
uint16_t handle_division(uint16_t A, uint16_t B, bool* overflow) {
  ...
  *overflow = false;
  ...
  uint32_t end_time = micros();
  uint32_t elapsed_time = end_time - start_time;
  printf("Division Time: %u microseconds\n", elapsed_time);
    ...
    while (true) {
      ...
      if (c == '\n') {
        if (currentLine.find("GET /calc?") != string::npos) {
          ...
          if (overflow) {
            cout << "Ocorreu Overflow ou Erro na operacao!" << endl;
          }
          cout << "-------------------------------\n" << endl;
        }

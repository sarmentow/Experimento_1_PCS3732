// Porte do 4_bit_calc.ino (ESP32) para rodar como programa nativo em um
...
//
// Compilar:  g++ -std=c++17 -O2 -o 4_bit_calc 4_bit_calc.cpp
// Executar:  ./4_bit_calc
...
void get_operation_params(const string& request, string* op, int8_t* A,
                           int8_t* B) {
...
void print_operation_params(const string& op, int8_t A, int8_t B) {
  ...
  printf("Operacao: %s\n", op.c_str());
  cout << "A: " << bitset<4>((uint8_t)A & 0x0F) << endl;
  cout << "B: " << bitset<4>((uint8_t)B & 0x0F) << endl;
}

int8_t handle_addition(int8_t A, int8_t B, bool* overflow) {
  int8_t result = A + B;
  *overflow =
      (((A & 0x08) == (B & 0x08)) && ((result & 0x08) != (A & 0x08)));
  return result;
}
...
int8_t handle_operation(const string& op, int8_t A, int8_t B,
                         bool* overflow) {
  int8_t result;
  if (op == "add") {
    ...
    while (true) {
      ...
      if (c == '\n') {
        if (currentLine.find("GET /calc?") != string::npos) {
          ...
          cout << "Resultado: " << bitset<4>((uint8_t)result & 0x0F)
               << endl;
          if (overflow) {
